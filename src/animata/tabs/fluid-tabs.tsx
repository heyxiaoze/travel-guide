/* Adapted from animata fluid-tabs (https://animata.design/docs/tabs/fluid-tabs)
 * for React 18 + shadcn/ui tokens (HSL). Changes vs upstream:
 *  - `use()` -> `useContext` (React 18)
 *  - tablist scrolls horizontally (`flex w-max` + `overflow-x-auto`) so many
 *    day-tabs fit on one row instead of compressing
 *  - tabs keep a comfortable minimum width with centered labels
 */
...
function FluidTabSlot({ index, children }: { index: number; children: ReactNode }) {
  ...
  return (
    <FluidTabSlotContext.Provider value={value}>{children}</FluidTabSlotContext.Provider>
  );
}
...
function FluidTabsList({
  ...
  return (
    <nav
      ...
      className={cn("thin-scroll w-full overflow-x-auto", className)}
      {...props}
    >
      <div className="relative w-max rounded-full bg-muted p-1 shadow-sm">
        <div
          role="tablist"
          tabIndex={0}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "relative z-10 flex min-w-[3.5rem] items-center justify-center px-3 py-2.5 text-sm font-semibold",
