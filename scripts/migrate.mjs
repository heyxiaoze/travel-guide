...
- \`guides/<id>.json\` —— 单篇指南完整数据（含 \`status: "draft" | "published"\`）
- \`guides/index.json\` —— 指南清单（id / status / 标题 / emoji / color / updatedAt 等），供站点枚举

站点构建期通过 \`raw.githubusercontent.com\` 拉取本仓库「已发布」指南生成静态快照；
管理员经 Cloudflare Pages Functions + GitHub PAT 写回本仓库并触发重新部署。
每篇指南的独立提交历史即为该指南的**版本历史**。
`;
